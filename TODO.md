...
- [x] **관리자 페이지(로컬 전용) 구현** — "장기 구상: 에이전트 분리 아키텍처" 원안
  1번. `pipeline/admin_server.py`(표준 라이브러리 `http.server`만 사용, 새 의존성 없음,
  인증 없음 — 로컬/1인 운영자 전제) + `admin/`(index.html/admin.js/admin.css, 빌드
  도구 없음). `data/tagged_articles.jsonl`을 산업/승인상태로 필터링·정렬해서 보고,
  기사별 `summary`/`why_it_matters`/`industries`를 편집할 수 있다. `industries`에
  `config/industries.json`에 없는 값(예: "로봇" 같은 한글 라벨이 잘못 들어간 경우 —
  `group_by_industry()`에서 조용히 누락되는 버그)이 있으면 빨간 배지로 표시. 실제
  버그 있던 레코드(인덱스 76/77, `["로봇", "모빌리티/자율주행"]`)를 이 도구로
  `["robot", "mobility"]`로 수정해 배지가 사라지는 것까지 확인.
- [x] **`approved` 플래그로 발행 게이트 강제** — 위 "장기 구상" 5번. 기사 레코드
  최상위에 `approved`(bool) 필드 추가, 관리자 페이지에서만 true로 바뀜(기본
  false/미설정). `config_loader.load_tagged_articles(only_approved=True)`를 추가하고
  `build_news.py`/`build_newsletter.py`가 이 옵션으로만 호출하도록 변경 — "완전 자동
  발행 금지" 원칙의 실제 집행 지점. 129건 중 3건을 승인/해제하며 `news.json`에 정확히
  반영/제외되는 것 실증 확인.
- [x] **수집+태깅 스케줄러** — 위 "장기 구상" 원안 2번(정의를 다듬은 결과, 3번 참고:
  AI 판단이 필요 없어 단순 스케줄러로 충분). `daily_report.ps1`과 같은 패턴
  (PowerShell 스크립트 + Windows 작업 스케줄러 + 로그 파일)으로
  `scripts/run_pipeline_scheduled.ps1` 작성, 매일 09:00 `run_pipeline.py`(수집+태깅)만
  실행하도록 `NewsletterProject-PipelineScheduler` 작업 등록(`daily_report.ps1`의
  23:00과 안 겹침). **`build_news.py`/`build_newsletter.py`/배포는 절대 호출하지
  않음** — 승인 게이트를 스케줄러가 우회하지 않도록 의도적으로 분리. 실제 1회 실행해
  RSS 수집(뉴스와이어 하루 5건 캡 정상 작동 확인) → 태깅 3건 성공 →
  `tagged_articles.jsonl` 반영(approved 기본값 미설정)까지 end-to-end 확인.
- [x] **API 회차당 상한선 가드 (신규)** — 위 "장기 구상" 4번의 세 번째 항목("API
  비용 — 사람 확인 없이 스케줄러가 계속 돌면 예산 초과 위험"). `config/pipeline_limits.json`에
  `max_articles_tagged_per_run: 50` 추가(정교한 비용 회계 아니라 단순 하드 캡, 무료
  티어 주간 필요량 30건보다 여유 있게 산정), `scrape_rss.collect_all()`이 이 값을
  넘는 신규 기사를 이번 회차에서 태깅 대상에서 제외하도록 수정. 넘는 만큼은
  `processed_urls.json`에도 기록하지 않아 다음 실행에서 자동으로 다시 후보가 됨(유실
  아님). 합성 데이터로 9건→5건 캡 + 미처리 4건 이월 동작 검증.
- 실행 방법은 `CLAUDE.md`의 "관리자 페이지"/"스케줄러" 절 참고.
- **부수 발견 + 복구**: 캡 로직을 실제 `processed_urls.json`/`newswire_daily_counts.json`으로
  직접 테스트하다가 실수로 두 파일을 삭제함. `newswire_daily_counts.json`은 날짜
  기준 자동 초기화라 문제 없었지만, `processed_urls.json`은 `tagged_articles.jsonl`의
  `link` 129개로 재구성해서 복구함 (완전히 동일하진 않을 수 있음 — 태깅 실패해서
  `tagged_articles.jsonl`엔 없지만 이전에 수집은 됐던 기사가 있었다면 그런 것만 다시
  후보가 될 수 있음, 큰 문제 아님). **교훈: 다음에 data/ 파일로 파괴적 테스트할 땐
  먼저 복사본을 떠 두거나 격리된 임시 디렉터리를 쓸 것.**
...
- [x] ~~뉴스와이어 "하루 5건" 한도, 코드로 강제 안 돼 있음 (2026-07-20 리뷰에서 발견)~~ —
  2026-07-20 완료. `config/sources.json`의 모든 뉴스와이어 rss 소스(14개)에
  `daily_cap: 5` 필드 추가. `scrape_rss.py`에 `data/newswire_daily_counts.json`(날짜 +
  소스 url 별 카운터, `processed_urls.json`과 같은 패턴 — `data/`라 gitignore 대상, 날짜가
  바뀌면 자동 초기화)을 신설하고 `collect_articles_for_industry()`가 소스별 `daily_cap`을
  강제하도록 수정: 이미 오늘 한도 도달한 소스는 fetch 자체를 건너뛰고, 한도 안에서만
  받은 기사를 잘라 쓰고 나머지는 로그로 알림. 같은 소스 url이 여러 산업에 매핑된 경우
  (예: "뉴스와이어 - 소프트웨어"가 embedded_iot/ai_software 둘 다에서 쓰임)에도 url을
  키로 써서 카운트가 합산되도록 함. 합성 RSS로 캡 초과 시나리오(1회차 8건 피드 → 5건만
  수집 + 스킵 로그, 2회차 같은 날 재실행 → fetch 자체 스킵)와 날짜 롤오버(다음날 카운트
  초기화) 둘 다 테스트 통과 확인. 표준 라이브러리만 사용, 새 의존성 없음.
...
## 장기 구상: 에이전트 분리 아키텍처 (2026-07-12 사용자 제안, 1·2번은 2026-07-20 완료)

지금은 사람(운영자)이 파이프라인 스크립트를 손으로 실행하고, **관리자 페이지에서**
`tagged_articles.jsonl`을 검수·승인한다(수동 JSONL 편집은 더 이상 안 함). 사용자가
제안한 다음 단계는 이 과정을 역할별로 나누는 것 — 아래는 원안 + 검토 후 다듬은
순서/제약이다 (2026-07-12, 원안 검토 반영).

**원안 4가지:**
1. ~~개발 에이전트가 관리자 페이지를 만든다~~ — **완료(2026-07-20)**. `admin/` +
   `pipeline/admin_server.py`. 카드뉴스 내용(요약/`why_it_matters`/산업 분류) 편집 +
   승인 플래그까지 구현. 위 "최근 완료 (2026-07-20, 추가분)" 참고.
2. ~~기사를 스크래핑해오는 에이전트~~ — **완료(2026-07-20)**. 아래 3번 검토대로 "AI가
   판단하는 에이전트"가 아니라 정해진 로직을 도는 **스케줄러**(`NewsletterProject-PipelineScheduler`,
   매일 09:00)로 구현. 수집+태깅까지만 자동, 승인/배포는 여전히 사람 몫.
3. 홈페이지 관리 에이전트 — 미착수. 승인된 기사를 `build_news.py`/`build_newsletter.py`/
   배포까지 자동으로 이어주는 단계인데, "완전 자동 발행 금지" 원칙과 정면으로 부딪히는
   지점이라(사람이 검수했어도 배포까지 자동화하면 마지막 확인 기회가 사라짐) 지금은
   의도적으로 손대지 않음. 몇 호를 "수집+태깅 자동 / 승인+배포 수동"으로 운영해보고
   여기서도 진짜 반복 노동이 확인되면 그때 다시 검토.
4. 홈페이지 관리 에이전트가 관리자 페이지를 통해 기사를 업로드 — 3번 미착수라 같이 보류.
